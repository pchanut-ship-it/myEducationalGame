...
const mImageWrap = document.getElementById('m-image-wrap');
...
function render() {
  ...
  grid.innerHTML = filtered.map(p => `
    <article class="card" data-id="${p.id}" tabindex="0">
      ${p.image ? `<div class="card-thumb"><img src="${p.image}" alt="${escapeHtml(p.title)}" loading="lazy"><span class="card-thumb-badge">${p.emoji} #${String(p.id).padStart(2, '0')}</span></div>` : ''}
      <div class="card-body">
        <div class="card-head">
...
function openModal(id) {
  ...
  if (p.image) {
    mImage.src = p.image;
    mImage.alt = p.title;
    mImageWrap.hidden = false;
  } else {
    mImage.removeAttribute('src');
    mImageWrap.hidden = true;
  }
  if (p.demo) {
    mDemo.href = p.demo;
    mDemo.hidden = false;
